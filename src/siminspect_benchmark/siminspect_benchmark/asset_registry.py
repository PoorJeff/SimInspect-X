...
def asset_from_dict(data):
    """Build a ROS asset message with explicit numeric field types."""
    a = Asset()
    a.id = data["id"]
    a.asset_type = data["asset_type"]
    mp = data["map_pose"]
    a.map_pose.position.x = float(mp["x"])
    a.map_pose.position.y = float(mp["y"])
    a.map_pose.position.z = float(mp["z"])
    yaw = float(mp["yaw"])
    a.map_pose.orientation.z = math.sin(yaw / 2)
    a.map_pose.orientation.w = math.cos(yaw / 2)
    a.min_value = float(data["gauge"]["min_value"])
    a.max_value = float(data["gauge"]["max_value"])
